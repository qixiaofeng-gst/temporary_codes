...
class WinUsbPy(object):
	...
	def list_usb_devices(self, **kwargs):
		...
		while self.api.exec_function_setupapi(SetupDiEnumDeviceInterfaces, self.handle, None, byref(self.guid), member_index, byref(sp_device_interface_data)):
			print(
				'==== just checking how many times looped.', i,
				sp_device_interface_data.cb_size, sp_device_interface_data.interface_class_guid.data1,
				self.guid.data1, hex(self.guid.data1)
			)
			self.api.exec_function_setupapi(
				SetupDiGetDeviceInterfaceDetail, self.handle, byref(sp_device_interface_data),
